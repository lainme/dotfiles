...
if !exists('TxtBrowser_Dict_Url')
    let TxtBrowser_Dict_Url = 'http://www.google.com/dictionary?aq=f&langpair=en|zh-CN&q=text&hl=en'
endif
...
"===================================================================
"Default map:
if ("" == mapcheck("<Leader>q", "n"))
    nmap <script> <silent> <unique> <Leader>q <ESC>:TQuery <cword> <CR>
endif
if ("" == mapcheck("<Leader>q", "v"))
    vmap <script> <silent> <unique> <Leader>q y<ESC>:TQuery <c-r>" <CR>
endif
...
command! -nargs=? -bar TQuery call s:TxtBrowserQuery(<f-args>)
...
function! s:TxtbrowserOpenUrl (url)
    ...
    call s:TxtBrowerSilent("!xdg-oen \"" . a:url . "\"")
endfunction
...
"使用multiquery来查询
function! s:TxtBrowserQuery (...) range
    if a:0 == 0
        let word = expand('<cword>')
    else
        let word = a:1
    ...
    if word == ""
        echohl ErrorMsg | echo "No text to query." | echohl Normal
        return -1
    else
        echo 'Quering: word' 
    endif
    
    call s:TxtBrowerSilent("!multiquery \"" . word . "\" > /dev/null &")
endfunction

"Silent处理
function! s:TxtBrowerSilent(cmdstring)
